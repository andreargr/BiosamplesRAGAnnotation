...
def plot_accuracies(models_data, model_names):
    ...
    ax.set_title('Ontology Precision by Method')
...
def main():
    df_comparison_RAG_4o_Bioportal_RAG = pd.read_csv("Bioportal_annotator_RAG/analysis/df_4o_mini_Bioportal_RAG.csv", header=0)
    df_comparison_4o_mini = pd.read_csv("df_gpt4o.csv",header=0)
    df_comparison_4o_mini_ft = pd.read_csv("df_ft_4o.csv",header=0)

    models_data = [df_comparison_4o_mini,df_comparison_4o_mini_ft,df_comparison_RAG_4o_Bioportal_RAG]
    model_names = ['Base GPT-4o-mini', 'Fine-tuned GPT-4o-mini', 'RAG Bioportal GPT-4o-mini']
